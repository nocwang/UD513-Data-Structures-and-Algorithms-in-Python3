# -*- coding: utf-8 -*-
"""Breadth_Depth First Search.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1p3zQro7LzNrHvfQpkWbOwvb5FZFuAW01

# Implementation of Breadth|Depth First Search with python 3 
...
Iterative approach:
Beging by picking a node and marking it as 'seen'. Common implementation places the seen nodes on a stack.
Pick an adjacent edge, follow it and mark the next node as seen by placing it on the seen stack.
Repeat as long as there are more edges and more unseen nodes.
If you come to a leaf or a node which has been seen before, trace back until you come to a node with an adjacent edge leading to a node you haven't seen and then follow the path searching deeper into the graph as before.
Recursive approach:
...
Dequeue the node (now that we have seen it and all its adjacent nodes).
Take the next node in the queue and repeat the process until the queue is empty.
...
# Depth First Search: Graph.dfs_helper and Graph.dfs
...
class Graph(object):
    ...
    def _clear_visited(self):
        """the value of the visited property of nodes in self.nodes are set to False"""
        for node in self.nodes:
            node.visited = False
...
# Test case to implement Breadth|Depth First Search, i.e., Graph.dfs, Graph.dfs_helper and Graph.bfs
